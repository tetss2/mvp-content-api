...
// Быстрые темы — индексируются числом, callback_data = "qt:0" (всегда < 64 байт)
const QUICK_TOPICS = [
  ...
  await bot.sendMessage(chatId,
    `🌟 *Готово! Выберите тему или напишите свою:*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: "💊 Либидо и как на него влиять", callback_data: "qt:0" },
            { text: "🔥 Оргазм: мифы и реальность", callback_data: "qt:1" },
          ],
          [
            { text: "💭 Сексуальные фантазии", callback_data: "qt:2" },
            { text: "⚡ Боль во время секса", callback_data: "qt:3" },
          ],
...
bot.on("callback_query", async (query) => {
  ...
  try {
    ...
    // Быстрые темы — индекс из QUICK_TOPICS
