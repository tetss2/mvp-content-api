...
const PRODUCTION_CURRENT = join(ROOT, "knowledge_indexes", "sexologist", "production", "current");
...
async function embedQuery(query) {
  ...
  if (!apiKey) throw new Error("OPENAI_API_KEY is required for production retrieval smoke test.");
...
async function searchFaiss(indexPath, queryEmbedding, topK) {
  const tempDir = join(os.tmpdir(), "mvp-content-api-faiss-production-test", timestampId());
...
`;
  try {
    ...
    if (result.status !== 0) {
      throw new Error(`FAISS production search failed: ${result.stderr || result.stdout || result.error?.message}`);
    }
...
async function main() {
  const manifestPath = join(PRODUCTION_CURRENT, "index_manifest.json");
  const productionManifestPath = join(PRODUCTION_CURRENT, "production_manifest.json");
  if (!(await exists(PRODUCTION_CURRENT))) throw new Error(`Missing production current: ${repoRelative(PRODUCTION_CURRENT)}`);
  if (!(await exists(manifestPath))) throw new Error(`Missing ${repoRelative(manifestPath)}`);
  const manifest = await readJson(manifestPath);
  if (manifest.kb_id !== "sexologist") throw new Error(`Expected sexologist production, got kb_id=${manifest.kb_id}`);
  ...
  const indexPath = join(PRODUCTION_CURRENT, manifest.faiss_index || "faiss.index");
  const docstorePath = join(PRODUCTION_CURRENT, manifest.docstore || "docstore.jsonl");
  const fallbackDocstorePath = join(PRODUCTION_CURRENT, "docstore.json");
  ...
  for (const required of [indexPath, resolvedDocstorePath]) {
    if (!(await exists(required))) throw new Error(`Missing production artifact: ${repoRelative(required)}`);
  }

  const productionManifest = await exists(productionManifestPath) ? await readJson(productionManifestPath) : null;
  ...
  const report = {
    type: "sexologist_production_retrieval_test",
    generated_at: nowIso(),
    safety: {
    ...
    production_dir: repoRelative(PRODUCTION_CURRENT),
    ...
    artifacts: {
      ...
      production_manifest: productionManifest ? repoRelative(productionManifestPath) : null,
    },
  ...
  const reportPath = join(REPORTS_DIR, `${timestampId()}_production_retrieval_test.json`);
  await writeJson(reportPath, report);

  console.log("Sexologist production retrieval smoke test");
  console.log(`Production: ${repoRelative(PRODUCTION_CURRENT)}`);
...
main().catch((err) => {
  console.error(`Sexologist production retrieval smoke test failed: ${err.message}`);
  process.exit(1);
});
